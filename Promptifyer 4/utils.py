...
def append_to_csv(df, csv_file, color):
...
def update_plot(data_path, plot_path):
    data = pd.read_csv(data_path)
    df = pd.DataFrame(data)

    plt.scatter(df['generation'], df['score'], c=df['color'].apply(ast.literal_eval))
...
def find_all_fittest(sentences_df, limit):
    df_sorted = sentences_df.sort_values(by='score', ascending=False)
    df_unique = df_sorted.drop_duplicates(subset=['sentence'])
    print(df_unique[['score', 'sentence']].head(limit))
    res = df_unique['sentence'].tolist()
    return res, df_unique
...
class WordRemover(ChildrenMaker):
    def __init__(self, num_removals):
        self.num_removals = num_removals

    def make_some_double_word_removals(self, parent):
        children = set()
        for i in range(self.num_removals):
            sentence = sentence_to_list(parent)
            w1 = random.choice(sentence)
            sentence.remove(w1)
            w2 = random.choice(sentence)
            sentence.remove(w2)
            children.add(list_to_sentence(sentence))
        return list(children)
    
    def make_some_triple_word_removals(self, parent):
        children = set()
        for i in range(self.num_removals):
            sentence = sentence_to_list(parent)
            w1 = random.choice(sentence)
    ...
    def make_children(self, parent):
        new_children = [parent]
        new_children += self.make_all_single_word_removals(parent)
        new_children += self.make_some_double_word_removals(parent)
        new_children += self.make_some_triple_word_removals(parent)
        len_lst = [len(sentence_to_list(child)) for child in new_children]
        print(f"Parent of length {len(sentence_to_list(parent))} made {len(new_children)} children with average length {np.mean(len_lst)}")
        return new_children